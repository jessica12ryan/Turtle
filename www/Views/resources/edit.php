    ...
    <form method="POST" action="/resources/<?= $link['id'] ?>/update">
        ...
        <div class="mb-6">
            ...
            <select name="type" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
                <option value="general" <?= ($link['type'] ?? 'general') === 'general' ? 'selected' : '' ?>><?= __('General Resource') ?></option>
                <option value="staff" <?= ($link['type'] ?? '') === 'staff' ? 'selected' : '' ?>><?= __('Staff Resource') ?></option>
            </select>
