<div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold text-black dark:text-white"><?= __('Staff') ?></h1>
    <div class="flex items-center space-x-3">
        <?php if (can('staff.archive') || can('staff.restore')): ?>
            <a href="/staff?show_archived=<?= $showArchived ? '0' : '1' ?>" data-tooltip="<?= __('Show or hide archived') ?>" class="text-sm <?= $showArchived ? 'bg-blue-100 text-blue-700' : 'text-gray-500 hover:text-gray-700' ?> px-3 py-1.5 rounded-lg border transition">
                <?= $showArchived ? __('Showing archived') : __('Show archived') ?>
            </a>
        <?php endif; ?>
        <?php if (can('staff.create')): ?>
            <a href="/staff/create" data-tooltip="<?= __('Add new') ?>" class="bg-yellow-600 text-white px-4 py-2 rounded-lg hover:bg-yellow-700 text-sm font-medium"><?= __('Add Staff') ?></a>
        <?php endif; ?>
    </div>
</div>
                        ...
                        <td class="px-6 py-4 space-x-2">
                            <?php if (!$s['archived_at']): ?>
                                <?php if (can('staff.edit')): ?>
                                    <a href="/staff/<?= $s['id'] ?>/edit" data-tooltip="<?= __('Edit this item') ?>" class="text-blue-600 hover:underline text-sm"><?= __('Edit') ?></a>
                                <?php endif; ?>
