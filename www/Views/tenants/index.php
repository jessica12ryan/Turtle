...
<div class="bg-white rounded-lg shadow overflow-hidden">
    <?php if (empty($tenants)): ?>
        <div class="p-6 text-center text-gray-500"><?= __('No tenants found.') ?></div>
    <?php else: ?>
        <table class="w-full">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase"><?= __('Phone') ?></th>
                    ...
                    <tr class="hover:bg-gray-50 <?= $tenant['archived_at'] ? 'opacity-60' : '' ?>">
                        <td class="px-6 py-4">
                            ...
                            <?php if ($tenant['is_main_tenant']): ?>
                                <span class="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded ml-1"><?= __('Main') ?></span>
                            <?php endif; ?>
                        ...
                        <td class="px-6 py-4">
                            <?php if ($tenant['moved_out_at']): ?>
                                <span class="text-xs bg-red-100 text-red-800 px-2 py-1 rounded"><?= __('Moved Out') ?></span>
                            <?php else: ?>
                                <span class="text-xs bg-green-100 text-green-800 px-2 py-1 rounded"><?= __('Active') ?></span>
                            <?php endif; ?>
                        </td>
