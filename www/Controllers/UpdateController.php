...
class UpdateController
{
    ...
    public function apply(): void
    {
        ...
        $steps = [
            'Fixing permissions...' => "chmod 755 {$repo} 2>&1 || { echo 'chmod repo failed' >&2; exit 1; }; chmod -R 775 {$repo}/storage {$repo}/.git {$repo}/www/assets/uploads 2>&1 || { echo 'chmod storage/.git failed (check ownership)' >&2; exit 1; }; if command -v sudo >/dev/null 2>&1; then sudo chmod -R 775 {$repo}/storage {$repo}/.git 2>&1 || echo 'sudo chmod failed, continuing without sudo' >&2; fi; rm -f {$repo}/.git/index.lock {$repo}/.git/FETCH_HEAD 2>&1; rm -rf {$repo}/storage/framework {$repo}/storage/logs 2>&1; true",
